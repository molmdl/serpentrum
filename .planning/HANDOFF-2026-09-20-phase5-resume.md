...
1. ~~**Dedicated debug session**~~ ✓ DONE 2026-09-20 — resolved (`64882c3` C1 demo-mode begin_game note + `e0b2814` C4 skip/refuse recap taxonomy + `2bff573` archive → `.planning/debug/resolved/05-upload-only-endless-run.md`). **C2 left as owner opt-in**: soft Apply-time warning in Setup status line (seam + patch sketch in the resolved debug file) — decide any time, cheap to add later.
